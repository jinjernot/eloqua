"""
Script to download all email HTML files from 2024 using Eloqua API
"""
import sys
import os
import requests
...
from config import BASE_URL, EMAIL_ASSET_ENDPOINT, EMAIL_DOWNLOADS_DIR
...
def fetch_all_email_assets_from_2024():
    """
    Fetches all email assets from Eloqua that were created or updated in 2024.
    ...
    while True:
        ...
        try:
            ...
            for email in emails:
                ...
                if updated_at and not is_from_2024:
                    try:
                        updated_date = datetime.fromtimestamp(int(updated_at))
                        if updated_date.year == 2024:
                            is_from_2024 = True
                    except (ValueError, TypeError):
                        pass
            ...
            logger.info(f"Processed page {page_count}, found {len(all_emails)} emails from 2024 so far...")
    ...
    logger.info(f"Found {len(all_emails)} total email assets from 2024")
    return all_emails


def download_all_2024_emails(max_workers=5):
    """
    Downloads HTML content for all emails from 2024.
    ...
    """
    # Fetch all email assets from 2024
    emails = fetch_all_email_assets_from_2024()
    
    if not emails:
        logger.warning("No emails found from 2024")
        return
    
    # Create output directory
    save_dir = f"{EMAIL_DOWNLOADS_DIR}/2024"
    ...
    logger.info(f"Total emails from 2024: {len(emails)}")
...
if __name__ == "__main__":
    ...
    download_all_2024_emails(max_workers=5)
