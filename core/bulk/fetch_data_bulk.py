...
def fetch_and_save_data(target_date=None):
    ...
    # Extend end date by +365 days to capture opens/forwards that happen up to 1 year after the send
    # This accounts for timezone differences and delayed opens/forwards/clicks
    # Extended from +180 to +365 days to capture full year engagement data
    engagement_start_date = start - timedelta(days=30)
    engagement_start_str = engagement_start_date.strftime("%Y-%m-%dT00:00:00Z")
    # Extend engagement window to +365 days after target date to capture late opens/forwards/clicks
    engagement_end_date = start + timedelta(days=366)  # +366 because start+1 is already next day start
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        future_to_key = {
            ...
            # Fetch opens/clicks by date - will filter to matching campaigns in process_data_bulk.py
